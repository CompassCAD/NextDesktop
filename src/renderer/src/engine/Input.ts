...
export class KeyboardEventCore {
  ...
  constructor(keyDown: boolean, key: number, fx: () => void, modifiers: KeyboardEventModifiers) {
...
export class KeyboardHandler {
  ...
  constructor() {
    this.keys = {
      ...
      END: 35,
      HOME: 36,
      NUM: function (n: number) {
        return n + 48
      },
      NUMPAD: function (n: number) {
        return n + 96
      }
    }
    this.lastKey = 0
  ...
  setLastKey(key: number): void {
    this.lastKey = key
  }
  defaultKeyDown(key: number): void {
    this.lastKey = key
  }
  addKeyEvent(
    keyDown: boolean,
    ...
    const keyEvent = new KeyboardEventCore(keyDown, key, fx, modifiers || { ctrl: false })
  ...
  onKeyUp(e: KeyboardEvent): void {
    ...
    for (let k = this.keyEvents.length - 1; k >= 0; k--) {
      if (this.keyEvents[k].key === e.which && !this.keyEvents[k].keyDown) {
        const needsCtrl = !!(this.keyEvents[k].modifiers && this.keyEvents[k].modifiers.ctrl)
  ...
  onKeyDown(e: KeyboardEvent): void {
    ...
    const ctrlHeld = e.ctrlKey || e.metaKey // native, always accurate
    ...
    // Never swallow browser/OS/devtools shortcuts (Ctrl/Cmd+anything) unless
    // we explicitly matched a registered ctrl-shortcut for this exact key.
    if (ctrlHeld && !matched) return

    for (let k = 0; k < this.defaultPreventionList.length; k++) {
      if (this.defaultPreventionList[k] === e.which) e.preventDefault()
    }
  }
  setKeyEnabled(key: number, enabled: boolean): void {
    let found = -1
    for (let i = 0; i < this.defaultPreventionList.length; i++) {
      ...
      if (found !== -1 && enabled) delete this.defaultPreventionList[i]

      if (found === -1 && !enabled) this.defaultPreventionList.push(key)
    }
  }
}
